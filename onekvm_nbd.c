...
#define trace_nbd_header_sent(...) do { } while (0)
#define trace_nbd_payload_sent(...) do { } while (0)
#define trace_nbd_header_received(...) do { } while (0)
#define trace_nbd_payload_received(...) do { } while (0)
...
static int nbd_set_recv_workqueue_nice(struct workqueue_struct *workq)
{
	...
	attrs->nice = MAX_NICE;
	get_online_cpus();
	error = apply_workqueue_attrs(workq, attrs);
	put_online_cpus();
...
static bool valid_max_cache_kb(unsigned int value)
{
	return value >= 128 && value <= 4096 && is_power_of_2(value);
}

static void apply_max_cache_kb(struct nbd_device *nbd, unsigned int value)
{
	struct request_queue *q = nbd->disk->queue;
	unsigned int sectors = value << 1;

	blk_queue_max_hw_sectors(q, sectors);
...
{
	struct nbd_device *nbd = dev_to_disk(dev)->private_data;

	return sysfs_emit(buf, "%u\n", nbd->max_cache_kb);
}

static ssize_t max_cache_kb_store(struct device *dev,
				  ...
				  const char *buf, size_t count)
{
	...
	if (err)
		return err;
	if (!valid_max_cache_kb(value))
		return -EINVAL;

	blk_mq_freeze_queue(nbd->disk->queue);
	apply_max_cache_kb(nbd, value);
	blk_mq_unfreeze_queue(nbd->disk->queue);
	return count;
}
static DEVICE_ATTR_RW(max_cache_kb);
...
static void nbd_dev_remove(struct nbd_device *nbd)
{
	...
	if (disk) {
		q = disk->queue;
		device_remove_file(disk_to_dev(disk), &dev_attr_max_cache_kb);
		del_gendisk(disk);
		blk_cleanup_queue(q);
...
static int nbd_start_device(struct nbd_device *nbd)
{
	...
	error = nbd_set_recv_workqueue_nice(nbd->recv_workq);
	if (error) {
		dev_err(disk_to_dev(nbd->disk),
			"Could not lower knbd recv work queue priority: %d.\n",
			error);
		destroy_workqueue(nbd->recv_workq);
		nbd->recv_workq = NULL;
		return error;
...
static int nbd_dbg_init(void)
{
	...
	dbg_dir = debugfs_create_dir("onekvm_nbd", NULL);
	if (!dbg_dir)
		return -EIO;
...
static int __init nbd_init(void)
{
	...
	if (!valid_max_cache_kb(max_cache_kb)) {
	...
	nbd_major = register_blkdev(0, "onekvm_nbd");
	if (nbd_major <= 0)
		return -EIO;

	if (genl_register_family(&nbd_genl_family)) {
		unregister_blkdev(nbd_major, "onekvm_nbd");
		return -EINVAL;
	}
	nbd_dbg_init();
...
MODULE_DESCRIPTION("OneKVM dedicated network block device");
MODULE_LICENSE("GPL");

module_param(nbds_max, int, 0444);
MODULE_PARM_DESC(nbds_max, "number of network block devices to initialize (default: 1)");
